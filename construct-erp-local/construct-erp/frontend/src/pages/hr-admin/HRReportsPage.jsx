...
function EmpMasterTab({ data, month, year, expiryDays, setExpiryDays }) {
  const d = data || {};
  const master = d.master || [];
  const newJoinees = d.new_joinees || [];
  const probation = d.probation || [];
  const separations = d.separations || [];
  const headcount = d.headcount || [];
  const contractExpiry = d.contract_expiry || [];
  const transfers = d.transfers || [];
  const docChecklist = d.document_checklist || [];
  ...
  const statusBadge = (v) => <span className="rounded-full bg-blue-50 px-2 py-1 text-xs font-black text-blue-700">{v || '-'}</span>;

  return (
    <div className="space-y-8">

      {/* 1. Employee Master List */}
      <ReportSection title="1. Employee Master List"
      ...
      {/* 2. New Joinees */}
      <ReportSection title={`2. New Joinee Report — ${MONTHS[month]} ${year}`}
        subtitle="Employees who joined in the selected month"
      ...
      {/* 3. Probation */}
      <ReportSection title="3. Employee Confirmation / Probation Report"
        subtitle="Employees currently on probation or due for confirmation"
      ...
      {/* 4. Separations */}
      <ReportSection title={`4. Employee Separation / Exit Report — ${MONTHS[month]} ${year}`}
        subtitle="Employees who exited in the selected month"
        action={<ExportBtn rows={separations} filename={`separations-${MONTHS[month]}-${year}.csv`} />}>
        <DataTable
          columns={[
            { key: 'employee_code', label: 'Code' },
            { key: 'name', label: 'Name' },
            ...
            { key: 'resignation_date', label: 'Resignation', render: (r) => fmtDate(r.resignation_date) },
            { key: 'last_working_date', label: 'LWD', render: (r) => fmtDate(r.last_working_date) },
          ]}
          rows={separations}
      ...
      {/* 5. Headcount Summary */}
      <ReportSection title="5. Headcount Summary Report"
      ...
      {/* 6. Contract Expiry */}
      <ReportSection title="6. Contract Expiry Alert Report"
        subtitle="Employees whose contracts expire within the window"
        action={
          <div className="flex items-center gap-3">
            ...
            <ExportBtn rows={contractExpiry} filename="contract-expiry-alert.csv" />
          </div>
        }>
        <DataTable
          columns={[
            ...
            { key: 'department_name', label: 'Dept', render: (r) => r.department_name || r.department || '-' },
            { key: 'employment_type', label: 'Type' },
            { key: 'contract_end_date', label: 'Contract End', render: (r) => fmtDate(r.contract_end_date) },
          ]}
          rows={contractExpiry}
          empty={`No contracts expiring in next ${expiryDays} days`}
        />
      </ReportSection>

      {/* 7. Transfers */}
      <ReportSection title="7. Employee Transfer Report"
        subtitle="Recent inter-department or inter-location transfers"
      ...
      <ReportSection title="8. Employee Document Checklist Report"
        subtitle={`Checklist of required documents: ${REQUIRED_DOCS.join(', ')}`}
        action={<ExportBtn rows={docChecklist} filename="document-checklist.csv" />}>
        <DataTable
          columns={[
            { key: 'employee_code', label: 'Code' },
            { key: 'name', label: 'Name' },
            ...
            ...REQUIRED_DOCS.map((doc) => ({
              ...
              label: doc,
              render: (r) => {
                const held = (r.documents || []).map((d) => String(d).toLowerCase());
                const ok = held.some((h) => h.includes(doc.toLowerCase().split(' ')[0]));
                return ok
...
export default function HRReportsPage() {
  ...
  const { data: empMasterData } = useQuery({
    queryKey: ['hr-report-emp-master', month, year, expiryDays],
    queryFn: () => API.get('/api/hr/reports/employee-master', { params: { month, year, expiry_days: expiryDays } }).then((r) => r.data?.data || {}),
    enabled: activeTab === 'emp-master',
  });
    ...
    <div className="min-h-screen p-6 space-y-6" style={{background:'#F8FAFC'}}>
      ...
      {loading ? (
        ...
        <>
          ...
          {activeTab === 'emp-master' && <EmpMasterTab data={empMasterData} month={month} year={year} expiryDays={expiryDays} setExpiryDays={setExpiryDays} />}
        </>
      )}
    </div>
  );
