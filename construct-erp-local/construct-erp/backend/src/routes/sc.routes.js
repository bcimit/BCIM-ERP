...
router.put('/work-orders/:id', authorize(...PLANNER), async (req, res) => {
  try {
    ...
    if (!r.rows.length) return res.status(404).json({ error: 'Not found' });
    // Update items if provided — preserve order exactly as sent from frontend
    if (Array.isArray(f.items) && f.items.length) {
      const incomingIds = f.items.filter(it => it.id).map(it => it.id);
      // Delete items not in the incoming list (only if unbilled)
      if (incomingIds.length > 0) {
        await query(
          `DELETE FROM sc_wo_items WHERE wo_id=$1 AND id != ALL($2) AND COALESCE(billed_qty,0)=0`,
          [req.params.id, incomingIds]
        );
      } else {
        await query(`DELETE FROM sc_wo_items WHERE wo_id=$1 AND COALESCE(billed_qty,0)=0`, [req.params.id]);
      }
      for (let k = 0; k < f.items.length; k++) {
        const it = f.items[k];
        if (it.id) {
          // Update existing item — preserve billed_qty, update everything else + sequence
          await query(
            `UPDATE sc_wo_items SET item_code=$1,description=$2,unit=$3,qty=$4,rate=$5,sequence_no=$6 WHERE id=$7 AND wo_id=$8`,
            [it.item_code||null, it.description, it.unit||null, it.qty||0, it.rate||0, k+1, it.id, req.params.id]
          );
        } else {
          // New item
          await query(
            `INSERT INTO sc_wo_items (wo_id,item_code,description,unit,qty,rate,sequence_no) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
            [req.params.id, it.item_code||null, it.description, it.unit||null, it.qty||0, it.rate||0, k+1]
          );
        }
      }
    }
