...
function tokens(name){return(name||'').toLowerCase().replace(/[^a-z0-9\s]/g,' ').split(/\s+/).filter(w=>w.length>1&&!STOP.has(w));}
function overlap(a,b){const ta=new Set(tokens(a)),tb=new Set(tokens(b));let h=0;for(const w of ta)if(tb.has(w))h++;return h;}

const MR_SERIAL = process.argv[2] || 'BCIM-TQS-BLR-MR-004';

(async () => {
  const c = await pool.connect();
  const mr = await c.query(`SELECT mr.id, mr.created_at::date AS mr_date FROM material_requisitions mr JOIN projects p ON p.id=mr.project_id WHERE LOWER(p.project_code)='wdiry0151' AND mr.serial_no_formatted=$1`,[MR_SERIAL]);
  if(!mr.rows.length){console.log('MR not found: '+MR_SERIAL);process.exit(1);}
  ...
  console.log('=== '+MR_SERIAL+' | date: '+mrDate+' ===');
  ...
  for(const r of pos.rows){
    ...
    for(const mi of mritems.rows){
      ...
      if(best.score>0){score+=best.score;pairs.push({mrMat:mi.material_name,mrQty:mritems.rows.find(x=>x.material_name===mi.material_name).quantity,mrUnit:mritems.rows.find(x=>x.material_name===mi.material_name).unit,poMat:best.poMat,poQty:best.poQty,poUnit:best.poUnit,s:best.score});}
    }
    if(score>0)results.push({poLabel:(r.serial_no_formatted||r.po_number),date:(r.po_date||'').toString().slice(0,10),score,pairs,mrsId:r.mrs_id});
  }
  results.sort((a,b)=>b.score-a.score);
  const top=results.filter(r=>r.score>=2);
  console.log('Matches (score >= 2):');
  console.log('─'.repeat(90));
  for(const res of top){
    console.log('\n'+res.poLabel+' | '+res.date+' | score: '+res.score+' | linked: '+(res.mrsId||'no'));
    res.pairs.sort((a,b)=>b.s-a.s).forEach(p=>console.log('  ['+p.s+'] MR: '+p.mrMat+' ('+p.mrQty+' '+p.mrUnit+') <-> PO: '+p.poMat+' ('+p.poQty+' '+p.poUnit+')'));
  }
  if(!top.length)console.log('No matches with score >= 2');
