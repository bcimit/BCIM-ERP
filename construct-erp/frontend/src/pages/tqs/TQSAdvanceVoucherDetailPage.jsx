...
function IssueModal({ voucher, onClose }) {
  ...
  const mutation = useMutation({
    mutationFn: () => tqsAdvanceAPI.issue(voucher.id, form),
    onSuccess: () => {
...
function RecoveryModal({ voucher, onClose }) {
  ...
  const { data: bills = [] } = useQuery({
    ...
    queryFn:  () => tqsAdvanceAPI.lookupBillsByVendor({
  ...
  const mutation = useMutation({
    mutationFn: () => tqsAdvanceAPI.recover(voucher.id, {
      ...form,
...
function EditPanel({ voucher, projects, onCancel, onSaved }) {
  ...
  const mutation = useMutation({
    mutationFn: () => tqsAdvanceAPI.update(voucher.id, form),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['tqs-advance', String(voucher.id)] });
...
export default function TQSAdvanceVoucherDetailPage() {
  ...
  const { data: voucher, isLoading, isError } = useQuery({
    ...
    queryFn: () => tqsAdvanceAPI.get(id).then(r => r.data?.data),
    staleTime: 5 * 60 * 1000,
  });
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              onClick={() => navigate(`/tqs/advances/${id}/print`)}
